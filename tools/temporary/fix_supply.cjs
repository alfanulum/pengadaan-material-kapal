...
const file = path.join(__dirname, 'resources', 'views', 'dashboards', 'supply.blade.php');
let content = fs.readFileSync(file, 'utf8');

// 1. Delete Alur Procurement block
const alurRegex = /<div class="bg-slate-50 border border-slate-200 rounded-3xl p-6 md:p-8 w-full max-w-sm">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/;
content = content.replace(alurRegex, '</div>');

// 2. Replace Grid Menu
const newMenu = `<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
...
const gridRegex = /<div class="grid grid-cols-1 md:grid-cols-2 gap-6">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/x-app-layout>/;
content = content.replace(gridRegex, newMenu + '\n        </div>\n    </div>\n</x-app-layout>\n');
