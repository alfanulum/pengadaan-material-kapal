<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">
                    Diskusi harga dan komersial dengan Supply Chain.
                </p>
            </div>
            <a href="{{ route('vendor.tenders.show', $invitation->id) }}"
                class="inline-flex items-center gap-2 px-5 py-2.5 bg-white text-slate-700 border border-slate-200 rounded-xl text-sm font-semibold hover:bg-slate-50 hover:text-amber-600 transition-all shadow-sm">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="flex flex-col lg:flex-row gap-6 h-[75vh] min-h-[600px]">
            ...
            <div class="w-full lg:w-1/3 flex flex-col gap-6">
                <div class="bg-gradient-to-br from-slate-900 via-amber-950 to-amber-900 rounded-3xl p-6 text-white shadow-lg shadow-amber-900/20 relative overflow-hidden flex-shrink-0">
                    <div class="absolute -top-24 -right-24 w-64 h-64 bg-amber-500/20 rounded-full blur-3xl"></div>
                    <div class="relative z-10">
                        <span class="inline-flex px-3 py-1 rounded-lg bg-white/10 border border-white/20 text-xs font-bold mb-4 backdrop-blur-sm text-amber-200">
                            MODE NEGOSIASI
                        </span>
                        <h3 class="text-xl font-bold leading-tight mb-2">
                            {{ $invitation->tender->nama_tender }}
                        </h3>
                        <p class="text-amber-100/80 text-sm font-mono">
                            {{ $invitation->tender->kode_tender }}
                        </p>
                    </div>
                </div>

                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex-1 flex flex-col">
                    ...
                    <div class="space-y-4 mb-6 flex-1">
                        <div>
                            <p class="text-xs text-slate-500 mb-1">Total Penawaran Awal</p>
                            @php
                                $quotation = \App\Models\VendorQuotation::where('tender_id', $invitation->tender_id)->where('vendor_id', $invitation->vendor_id)->first();
                            @endphp
                            <p class="text-base font-bold text-slate-900">
                                @if($quotation)
                                    Rp {{ number_format($quotation->harga_penawaran, 0, ',', '.') }}
                                @else
                                    <span class="text-slate-400 font-normal italic">Belum menawar</span>
                                @endif
                            </p>
                        </div>
                        @if($quotation && $quotation->harga_negosiasi)
                        <div class="bg-amber-50 p-3 rounded-xl border border-amber-100">
                            <p class="text-xs text-amber-600 font-bold mb-1">Nominal Negosiasi Saat Ini</p>
                            <p class="text-lg font-bold text-amber-700">Rp {{ number_format($quotation->harga_negosiasi, 0, ',', '.') }}</p>
                        </div>
                        @endif
            ...
            <div class="w-full lg:w-2/3 bg-white rounded-3xl shadow-sm border border-slate-200 flex flex-col overflow-hidden relative">
                ...
                <div class="px-6 py-4 border-b border-slate-100 bg-white/80 backdrop-blur-md z-10 flex items-center justify-between shadow-sm">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-slate-900">Ruang Negosiasi Harga</h3>
                ...
                <div id="chatBox" class="flex-1 p-6 overflow-y-auto bg-slate-50/50 space-y-6">
                        ...
                        @if ($msg->sender_id == auth()->id())
                            ...
                            <div class="flex justify-end mb-4 group" data-msg-id="{{ $msg->id }}">
                                <div class="max-w-[75%] lg:max-w-[65%] flex flex-col items-end">
                                    ...
                                    <div class="bg-amber-600 text-white px-5 py-3.5 rounded-2xl rounded-tr-sm shadow-sm">
                                        @if ($msg->attachment)
                        ...
                        @else
                            ...
                            <div class="flex justify-start mb-4 group" data-msg-id="{{ $msg->id }}">
                                <div class="max-w-[75%] lg:max-w-[65%] flex flex-col items-start">
                                    <span class="text-[10px] font-bold text-slate-400 mb-1 px-1">Supply Chain</span>
                    ...
                    @empty
                        <div class="flex flex-col items-center justify-center h-full text-center opacity-50 py-10">
                            ...
                            <p class="text-slate-600 font-bold">Mulai Negosiasi</p>
                            <p class="text-xs text-slate-500 mt-1 max-w-xs">Diskusikan harga penawaran dan persyaratan komersial lainnya dengan Supply Chain.</p>
                        </div>
                    @endforelse
                ...
                <div class="p-4 bg-white border-t border-slate-100 z-10">
                    <form action="{{ route('vendor.tenders.chat.negotiation.send', $invitation->id) }}" method="POST" id="chatForm" enctype="multipart/form-data" class="relative">
                        @csrf
                        ...
                        <div class="flex items-end gap-2 bg-slate-50 p-2 rounded-2xl border border-slate-200 focus-within:border-amber-300 focus-within:ring-4 focus-within:ring-amber-50 transition-all">
                            
                            <label class="shrink-0 p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl cursor-pointer transition">
                            ...
                            <textarea name="message" id="messageInput" rows="1" 
                                ...
                                placeholder="Ketik penawaran / negosiasi..."></textarea>

                            <button type="submit" id="sendBtn" class="shrink-0 p-3 bg-amber-600 text-white rounded-xl hover:bg-amber-700 transition shadow-sm shadow-amber-600/30">
    ...
    (function () {
        ...
        const scrollBottom = () => { if(chatBox) chatBox.scrollTop = chatBox.scrollHeight; };
        ...
        const buildBubble = (msg, tempId = null) => {
            ...
            if (isMe) {
                ...
                <div class="flex justify-end mb-4 group" ${id}>
                    <div class="max-w-[75%] lg:max-w-[65%] flex flex-col items-end">
                        <span class="text-[10px] font-bold text-slate-400 mb-1 px-1">Anda</span>
                        <div class="bg-amber-600 text-white px-5 py-3.5 rounded-2xl rounded-tr-sm shadow-sm ${tempId ? 'opacity-70' : ''}">
                            ${attachmentHtml}
        ...
        if (sendForm && attachmentInput) {
            ...
            sendForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (res.ok) {
                        el.querySelector('div > div.bg-amber-600')?.classList.remove('opacity-70');
                    } else {
                        el.remove();
        ...
        const poll = async () => {
            ...
            try {
                ...
                fresh.forEach(msg => {
                    ...
                    if (msg.role === 'other') {
                        toast('💰 Negosiasi: Supply Chain', msg.message.substring(0, 50));
                        if(!sendForm) setTimeout(() => { window.location.reload(); }, 1000);
                    }
                });
            } catch (e) { }
